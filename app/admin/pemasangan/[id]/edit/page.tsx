...
export default function EditPemasanganPage() {
  ...
  async function fetchData() {
    try {
      ...
      const item = (data.items || []).find((i: any) => i.id === parseInt(id));
      if (item) {
        setNamaTempat(item.nama_tempat);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/hasil-pemasangan?id=${id}`, {
        method: 'PUT',
    ...
    <div>
      <div className="min-h-screen bg-background pt-24 pb-20">
        ...
        <div className="container-custom relative z-10 max-w-3xl mx-auto">
          ...
          <h1 className="text-2xl font-black text-white mb-8">Edit Hasil Pemasangan</h1>
